...
/*   Updated: 2024/07/22 15:11:45 by rrichard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

t_bool	getfile(t_parse *list)
{
	if (list->infile_token && !ft_strncmp(list->infile_token, "<<", 2))
		call_heredoc(list);
	else if (list->infile_token && !ft_strncmp(list->infile_token, "<", 1))
	{
		if (list->infile_access)
			list->infile = open(list->infile_name, O_RDONLY);
		else
		{
	...
	if (list->outfile_token && !ft_strncmp(list->outfile_token, ">>", 2))
		list->outfile = open(list->outfile_name, O_RDWR | O_APPEND, 0644);
	else if (list->outfile_token)
	{
		if (list->outfile_token && !list->outfile_name)
		{
			perror(list->outfile_token);
			return (FALSE);
		}
		else if (list->outfile_token && !ft_strncmp(list->outfile_token, ">", 1))
			list->outfile = open(list->outfile_name, O_WRONLY | O_TRUNC | O_CREAT, 0644);
	}
	return (TRUE);
}
